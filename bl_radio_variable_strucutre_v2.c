...
//                Semaphore_delete(&semTxHandle);
//                Semaphore_delete(&semRxHandle);
//                Semaphore_delete(&semUartRxHandle);
//                Semaphore_destruct(&semTxStruct);
//                Semaphore_destruct(&semRxStruct);
//                Semaphore_destruct(&semUartRxStruct);
//                Task_destruct(&uartRxTask);
//                Task_destruct(&uartTxTask);
//                Task_destruct(&rxTask);
//                Task_destruct(&txTask);
                saltProgram();
                break;
...
//        pkt.cmd = input;
        ...
        case 0x11:
            ...
            txPacket[1] = 0x46;
            memcpy(&txPacket[2], &PaketTx.pkt_payload, sizeof(PaketTx.pkt_payload));
            charIndex = 0;
        ...
        case 0x19:
            RF_cancelCmd(rfHandle, rfRxCmd, 0);
            PaketTx.pkt_one.cmd = input;
            payload_len = 2 + sizeof(PaketTx.pkt_one);
            txPacket[1] = 0x50;
            memcpy(&txPacket[2], &PaketTx.pkt_one, sizeof(PaketTx.pkt_one));
            charIndex = 0;
            Semaphore_post(semTxHandle);
            break;
        case 0x01:
...
void callback(RF_Handle h, RF_CmdHandle ch, RF_EventMask e)
{
    ...
    if (e & RF_EventRxEntryDone)
    {
        ...
        } else if (paketType == 0x50){
            memcpy(&PaketRx.pkt_one, packetDataPointer, sizeof(PaketRx.pkt_one));
            PaketRx.pkt_one.targetAddress = targetAddress;
        }
