...
static void uartTxTaskFunction(UArg arg0, UArg arg1)
{
    ...
    while (1) {
        ...
        switch(input){
        case 0x10:
            UART_read(uart, &txPacket[1], 4);
            UART_read(uart, &txPacket[5], 4);
            RF_cancelCmd(rfHandle, rfRxCmd, 0);
            Semaphore_post(semTxHandle);
            charIndex = 0;
            break;
        case 0x11:
            UART_read(uart, &txPacket[charIndex++], 1);
            dim_paket = txPacket[1];
            while(dim_paket--){
                UART_read(uart, &txPacket[charIndex++], 1);
                //txPacket[charIndex++] = input;
            }
...
static void uartRxTaskFunction(UArg arg0, UArg arg1)
{
    ...
    while (1) {
        ...
        if (packetReady) {
            writePayLoad(packetDataPointer, packetLength);
            packetReady = 0;
        }
    }
}
...
void writePayLoad(uint8_t *buffer, uint8_t length) {
    ...
    case 0x29:
        UART_write(uart, &flash_address, sizeof(flash_address));
        UART_write(uart, &app_size, sizeof(app_size));
        break;

    case 0x10:
        flash_address |= (uint32_t)buffer[1] << 24;
        flash_address |= (uint32_t)buffer[2] << 16;
        flash_address |= (uint32_t)buffer[3] << 8;
        flash_address |= (uint32_t)buffer[4];

        app_size |= (uint32_t)buffer[5] << 24;
        app_size |= (uint32_t)buffer[6] << 16;
        app_size |= (uint32_t)buffer[7] << 8;
        app_size |= (uint32_t)buffer[8];
//        System_printf("%d\t%d\n", flash_address, app_size);
//        System_flush();
//        UART_write(uart, &flash_address, sizeof(flash_address));
//        UART_write(uart, &app_size, sizeof(app_size));
        break;
        ...
        while(dim_paket--){
            if(i == 4096 * flash_erased){
                if(eraseFlashSector(flash_address + (4096 * flash_erased)) < 0){
                    //System_printf("error erasing flash");
                    //System_flush();
            ...
            if(flashReadyFSM()){
                if(writeFlash(flash_address + i, &buffer[count], 1, false) < 0){
